...
import {Avatar, Button, Space, Card, Col, Divider, Input, Row, Tag, Form, Alert, message} from 'antd';
...
class regionExample extends React.Component <any, any>{
  ...
  async addInput(){
    ...
    {data=this.state.center.lng+"_"+this.state.center.lat;}
    else{
      this.state.Point.map((e:any)=>(
        data +=e.lng+"_"+e.lat+";"));
      console.log(data);
    }

    let result =await request<String>('/fence/region/addMap', {
      method: 'POST',
      data: {    
        regionEntity:{
          name:this.state.name,
          type:this.state.type,
          color:this.state.radius
